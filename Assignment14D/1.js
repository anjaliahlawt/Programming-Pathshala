//Find the frequency of each tag. (There are 3 recipes where “Italian” tag is present, so frequency of Italian tag is 3)
...
for (let i = 0; i < data.recipes.length; i++) 
    {
      const tags = data.recipes[i].tags;
      for (let j = 0; j < tags.length; j++) 
        {
          // const tag = tags[j];
          if (result[tags[j]] === undefined) 
              {
                result[tags[j]] = 1;
              } 
          else 
...
// const result = data.recipes.reduce((acc, item) => {
