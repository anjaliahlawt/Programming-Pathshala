...
//   {
//    result.push({ name: data.recipes[i].name, cuisine: data.recipes[i].cuisine,rating:data.recipe[i] .  rating});
//       }
//     }
...
//     if(item.rating>4.5)
//         {
//             result.push({ name:item.name, cuisine:item.cuisine,rating:item.rating});
//         }
// });
// console.log(result);
...
//     .map(item => ({ name: item.name, cuisine: item.cuisine, rating: item.rating }))
...
const result = data.recipes.flatMap(item => {
    return item.rating > 4.5 ? [{ name: item.name, cuisine: item.cuisine }] : [];
});
